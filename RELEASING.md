...
- [ ] **Record changes under `## [Unreleased]`** in [`CHANGELOG.md`](CHANGELOG.md)
      (*Keep a Changelog* format). `/do-release` drafts these entries by running
      **`loom report release-notes`** — which selects the Unreleased done plans (roadmap history
      where `release: null`), enriches them from their done docs, and frames Highlights →
      Added / Changed / Fixed; `git log` is only a coverage net. Review and edit the draft before
      the bump rolls it into the dated `## [X.Y.Z]` section that becomes the GitHub release body.

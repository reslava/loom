...
| `report` | `kind` (weave / thread / date / `full` / `sort` / `titlesOnly` / `forward` / `creativity` optional) | Assembles a doc-graph slice for a report kind (selection chosen by the kind's `source`) and instructs the agent to synthesize + persist it via `loom_create_report`. **`release-notes`** is enriched: it selects the *Unreleased* done plans (`actual_release` null), hydrates their done-doc bodies (`titlesOnly` skips this for a fast draft), and on an empty set returns a **"NOTHING UNRELEASED"** stop-signal (naming threads still `implementing`) instead of a draft. **`next-work`** is prospective (`source: forward-signal`): it mines the graph's open material (`buildForwardSignal`) into a ranked, cited next-work list, with its own empty-set stop-signal. `forward` reframes any retrospective kind prospectively; `creativity` (`closed`\|`creative`) sets the solution latitude of prospective output. |

...
steps:
  - id: add-a-helper-function-if-not
    order: 1
    status: done
    description: Add a helper function (if not present) to read the user name and AI model name from `settings.json` — for example, `getUserName()` and `getAiName()` — in a shared utility module.
    files_touched: []
    blocked_by: []
    satisfies: []
  - id: in-packages-app-src-chatnew
    order: 2
    status: done
    description: "In `packages/app/src/chatNew.ts` line 45, replace the hardcoded `## Rafa:` with the dynamically fetched user name using the helper."
    files_touched: []
    blocked_by: []
    satisfies: []
  - id: in-packages-app-src-chatreply
    order: 3
    status: done
    description: "In `packages/app/src/chatReply.ts` lines 16 and 117, replace the hardcoded `## AI:` with the dynamically fetched AI model name using the helper."
    files_touched: []
    blocked_by: []
    satisfies: []
  - id: verify-that-the-settings
    order: 4
    status: done
    description: Verify that the settings.json is loaded before these strings are used (e.g., during app initialization) to avoid undefined values at runtime.
    files_touched: []
    blocked_by: []
    satisfies: []
---

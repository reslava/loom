...
steps:
  - id: setup-filesystem-utilities-with-robust-loom
    order: 1
    status: done
    description: Setup filesystem utilities (with robust loom resolution)
    files_touched: ["`packages/fs/src/utils.ts`"]
    blocked_by: []
    satisfies: []
  - id: implement-markdown-frontmatter-validation
    order: 2
    status: done
    description: Implement `loadDoc` (Markdown + frontmatter + validation)
    files_touched: ["`packages/fs/src/load.ts`"]
    blocked_by: [Step 1]
    satisfies: []
  - id: implement-atomic-markdown-writer
    order: 3
    status: done
    description: Implement `saveDoc` (atomic Markdown writer)
  ...
  - id: implement
    order: 4
    status: done
    description: Implement `loadThread`
  ...
  - id: implement-2
    order: 5
    status: done
    description: Implement `saveThread`
    files_touched: ["`packages/fs/src/saveThread.ts`"]
    blocked_by: [Steps 2, 3]
    satisfies: []
  - id: integrate-with-core-engine
    order: 6
    status: done
    description: Integrate with core engine (`runEvent`)
    files_touched: ["`packages/fs/src/runEvent.ts`"]
    blocked_by: [Steps 4, 5]
    satisfies: []
  - id: test-with-real-thread-folder
    order: 7
    status: done
    description: Test with real thread folder
    files_touched: ["`looms/test/threads/example/`"]
    blocked_by: [All]
    satisfies: []
---

# Filesystem Integration — Markdown Load and Save

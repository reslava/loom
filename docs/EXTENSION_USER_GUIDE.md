...
A **Feedback** button in the status bar (and the `Loom: Send Feedback` command) opens a prefilled GitHub issue about Loom. It's **opt-in** and carries only Loom version, OS, and non-PII usage counts (weaves / threads / done plans / current release) — you review and edit everything before submitting; nothing is sent in the background.

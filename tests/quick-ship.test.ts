...
async function makeThread(loomRoot: string, weaveSlug: string, slug: string): Promise<string> {
    const { id } = await createThread(
        { weaveSlug: weaveSlug, threadSlug: slug },
...
async function findPlan(loomRoot: string, weaveSlug: string, planId: string) {
...
async function run() {
    ...
    {
        const loomRoot = await makeLoomRoot();
        const weaveSlug = 'qs-weave-b';
        const threadUlid = await makeThread(loomRoot, weaveSlug, 'fast-fixes');
        const result = await quickShip(
            { weaveSlug: weaveSlug, threadUlid, description: ['did A', 'did B', 'did C'] },
            makeDeps(loomRoot),
        );
    ...
    {
        const loomRoot = await makeLoomRoot();
        const weaveSlug = 'qs-weave-c';
        const result = await quickShip(
            { weaveSlug: weaveSlug, newThread: { slug: 'standalone-fix', title: 'Standalone fix' }, description: 'one-off fix' },
            makeDeps(loomRoot),
        );
        ...
        const threadManifest = path.join(loomRoot, 'loom', weaveSlug, 'standalone-fix', 'thread.md');
    ...
    {
        const loomRoot = await makeLoomRoot();
        const weaveSlug = 'qs-weave-d';
        const weavePath = path.join(loomRoot, 'loom', weaveSlug);
        const existingPlanId = `${weaveSlug}-plan-001`;
        ...
        const threadUlid = await makeThread(loomRoot, weaveSlug, 'qs-thread-d');
        const result = await quickShip(
            { weaveSlug: weaveSlug, threadUlid, description: 'a side fix that arose mid-plan' },
            makeDeps(loomRoot),
        );
        assert(result.planId !== existingPlanId, 'quick-ship must mint a NEW plan, not reuse the existing one');
        const qsPlan = await findPlan(loomRoot, weaveSlug, result.planId);
    ...
    {
        const loomRoot = await makeLoomRoot();
        const weaveSlug = 'qs-weave-e';
        let threw = 0;
        try { await quickShip({ weaveSlug: weaveSlug, description: 'x' } as any, makeDeps(loomRoot)); } catch { threw++; }
        try { await quickShip({ weaveSlug: weaveSlug, threadUlid: 't',newThread: { slug: 's' }, description: 'x' }, makeDeps(loomRoot)); } catch { threw++; }
        try { await quickShip({ weaveSlug: weaveSlug, threadUlid: 't',description: '   ' }, makeDeps(loomRoot)); } catch { threw++; }
        try { await quickShip({ weaveSlug: weaveSlug, threadUlid: 't',description: [] }, makeDeps(loomRoot)); } catch { threw++; }

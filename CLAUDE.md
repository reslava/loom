...
**Tri-surface command parity (hard).** A capability is exposed on **every surface its consumer needs**, with **names mirrored** across **CLI ⇄ MCP ⇄ extension**. Touching a command on one surface obliges you to *consider and mirror* it on the others **in the same change**: CLI `foo-bar` ⇄ MCP `loom_foo_bar` ⇄ extension menu "Foo Bar" (the subcommand *form* may differ — `loom create idea` mirrors `loom_create_idea` — since the CLI is human-first). Exceptions are **by consumer, not by whim**: (a) *form* differences — CLI slug/human-first vs MCP ULID-strict; (b) genuinely single-audience commands stay single-surface — **agent-only** workflow tools (`loom_do_step`, `loom_read_chat_tail`, `loom_append_done`, `loom_patch_doc`) need no CLI/menu twin, and **setup** like `loom install` needs no MCP twin. "Consider parity" is the rule; blanket triplication is not. **Availability clause:** every way advertised in [docs/WAYS-TO-USE-LOOM.md](docs/WAYS-TO-USE-LOOM.md) must be fully runnable on its surface — in particular the extension-free **Pure agent** way needs a CLI twin for every human doc-management op the extension exposes as a tree button (create/delete/archive/rename/move/reorder). Verbs must also *match*: a `weave`-vs-`create` split across surfaces is a parity bug, not a style choice. *(Repo-specific authoring rule — no `rule:` marker, not mirrored to the template.)*
...
- Use `loom://context/{docUlid}` (or `loom://context/thread/{weaveSlug}/{threadSlug}`) before starting any thread work. The Unified Context Pipeline bundles everything the agent needs (global ctx, idea, design, active plan, requires_load refs) in a single read.
...
- doc-only (active thread): once a thread is the **active thread**, point at one of its docs with `?scope=doc` — `loom://context/{weaveSlug}/{threadSlug}/{docSlug}?scope=doc` returns only that doc, not the whole bundle you already hold (the `read`/`reply` slang path). The first pointed action of a session loads the full thread (`load`); single-doc reads after that are doc-only.
...
- `load {weaveSlug}/{threadSlug}` — load `loom://context/thread/{weaveSlug}/{threadSlug}` (the full thread bundle) and make it the **active thread**. The only verb that sets/switches it — it lives in *your* context, not the stateless server. Emit `🧵 Active: {weaveSlug}/{threadSlug}`. The first pointed action of a session implicitly loads its thread.
- `read {docSlug}` *(active thread)* / `read {weaveSlug}/{threadSlug}/{docSlug}` — doc-only: `loom://context/{weaveSlug}/{threadSlug}/{docSlug}?scope=doc` returns **only that doc** (`?mode=chat` for a chat), never re-bundling the active thread you already hold. Bare `{docSlug}` resolves inside the active thread. After the first touch `read` **never auto-loads or switches** — a cross-thread `read` is a pure peek. **If the target is a chat doc with a pending `## Rafa:` turn (one after the last `## AI:`), `read` implies `reply`** — flow straight into the reply chain. Reading a non-chat doc, or a chat whose last turn is already `## AI:`, stays load-only.
- `reply` *(a chat doc is active)* — `loom_read_chat_tail` → compose → `loom_append_to_chat`. `reply {chatSlug}` resolves the chat in the active thread; `reply {weaveSlug}/{threadSlug}/{chatSlug}` is honored **only for the active thread** — a non-active thread is refused (prompt to `load` it first).
...
Slang covers only ambiguous or multi-step verbs; a capability with a self-naming command (`set-status`, `rename`, `archive`, …) you run by that name. No single-letter aliases.

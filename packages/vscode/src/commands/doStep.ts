...
function buildPrompt(planId: string, stepNumbers: number[]): string {
    if (stepNumbers.length === 1) {
        return `Use the loom MCP server. Call loom_do_step with plan_ulid="${planId}" and stepNumber=${stepNumbers[0]} to get the brief. Implement the step using your tools. After implementation, call loom_append_done with your notes, then loom_complete_step. Show me what you did.`;
    }
    const list = JSON.stringify(stepNumbers);
    return `Use the loom MCP server to implement multiple plan steps in one session, preserving context across them. plan_ulid: "${planId}". Step numbers in dependency order: ${list}. Implement ONLY these steps — do not implement any other steps even if they appear not-done in the plan summary or seem like prerequisites. If a step cannot be completed without an unlisted step, stop and report the issue instead of resolving it yourself. For each step number in this exact order: (1) call loom_do_step with plan_ulid and stepNumber, (2) implement the step using your tools, (3) call loom_append_done with your notes for that step, (4) call loom_complete_step for that step. Then move to the next number without exiting. If any step fails, stop and report which step failed and why — do not skip ahead. Summarise what you did per step at the end.`;
}

...
export async function promoteToIdeaCommand(treeProvider: LoomTreeProvider, node?: TreeNode): Promise<void> {
    ...
    const toolArgs: Record<string, unknown> = { source_ulid: sourceId, targetType: 'idea' };
    ...
    // Resolve the target thread to its ULID (minting the manifest for a new thread).
    ...
    if (await isClaudeInstalled()) {
        ...
        await launchClaude(root, `Loom: Promote to Idea`,
            `Loom promote to idea task. source_ulid="${sourceId}", target_weave_slug="${targetWeaveId}"${targetThreadUlid ? `, target_thread_ulid="${targetThreadUlid}"` : ''}. ${readInstruction} Then call MCP tool loom_create_idea ONCE with weave_slug="${targetWeaveId}"${targetThreadUlid ? `, thread_ulid="${targetThreadUlid}"` : ''}, a concise title, and content (the full idea body derived from the source). Do NOT call loom_update_doc afterwards — pass the body in the content argument of loom_create_idea, in the same single call. Do not use loom_promote — sampling is unavailable in Claude Code CLI. Do not invoke CLI commands via Bash.`
        );
    } else {
        try {
            let result: any;

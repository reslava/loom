...
export async function promoteToDesignCommand(treeProvider: LoomTreeProvider, node?: TreeNode): Promise<void> {
    ...
    if (!sourceId) {
        vscode.window.showErrorMessage('Right-click a chat, idea, or doc in the tree to promote it.');
        return;
    }
    ...
    try {
        ...
        await vscode.window.withProgress(
            ...
            async () => {
                result = await getMCP(root).callTool('loom_promote', { sourceId, targetType: 'design' });
            }
        );

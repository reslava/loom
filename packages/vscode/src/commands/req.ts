...
export async function generateReqCommand(
    ...
    const threadSlug = node?.threadSlug;
    const threadUlid = node?.threadUlid;
    if (!weaveSlug || !threadSlug) { vscode.window.showErrorMessage('Right-click a thread to generate its requirements.'); return; }

    if (await isClaudeInstalled()) {
        if (!threadUlid) { vscode.window.showErrorMessage(`Thread '${threadSlug}' has no thread.md manifest — cannot create its req by identity.`); return; }
        await launchClaude(root, 'Loom: Generate Requirements',
            `Loom generate requirements task. weave_slug="${weaveSlug}", thread_ulid="${threadUlid}". Use the loom MCP server: read the thread's chat context (resource loom://context/thread/${weaveSlug}/${threadSlug}?mode=chat), then call MCP tool loom_create_req ONCE with weave_slug="${weaveSlug}" thread_ulid="${threadUlid}" and content — ${REQ_BODY_SHAPE} Do NOT call loom_update_doc afterwards — pass the body in the content argument. Do not use loom_generate_req — sampling is unavailable in Claude Code CLI.`,
        );
    } else {
        try {
            let result: any;
            await vscode.window.withProgress(
                ...
                async () => { result = await getMCP(root).callTool('loom_generate_req', { weave_slug: weaveSlug, thread_ulid: threadUlid }); },
            );
...
export async function verifyReqCommand(treeProvider: LoomTreeProvider, node?: TreeNode): Promise<void> {
    ...
    const threadSlug = node?.threadSlug;
    const threadUlid = node?.threadUlid;
    if (!weaveSlug || !threadSlug) { vscode.window.showErrorMessage('Right-click a req (or its thread) to verify the plan against it.'); return; }
    if (!threadUlid) { vscode.window.showErrorMessage(`Thread '${threadSlug}' has no thread.md manifest — cannot verify its req by identity.`); return; }

    try {
        let result: any;
        await vscode.window.withProgress(
            { location: vscode.ProgressLocation.Notification, title: 'Loom: Verifying plan against requirements…', cancellable: false },
            async () => { result = await getMCP(root).callTool('loom_verify_req', { weave_slug: weaveSlug, thread_ulid: threadUlid }); },
        );
...
export async function finalizeReqCommand(treeProvider: LoomTreeProvider, node?: TreeNode): Promise<void> {
    ...
    const threadSlug = node?.threadSlug;
    const threadUlid = node?.threadUlid;
    if (!weaveSlug || !threadSlug) { vscode.window.showErrorMessage('Right-click a req doc to finalize it.'); return; }
    if (!threadUlid) { vscode.window.showErrorMessage(`Thread '${threadSlug}' has no thread.md manifest — cannot finalize its req by identity.`); return; }

    try {
        await getMCP(root).callTool('loom_finalize_req', { weave_slug: weaveSlug, thread_ulid: threadUlid });
...
export async function amendReqCommand(treeProvider: LoomTreeProvider, node?: TreeNode): Promise<void> {
    ...
    const threadSlug = node?.threadSlug;
    const threadUlid = node?.threadUlid;
    if (!weaveSlug || !threadSlug) { vscode.window.showErrorMessage('Right-click a req doc to amend it.'); return; }
    if (!threadUlid) { vscode.window.showErrorMessage(`Thread '${threadSlug}' has no thread.md manifest — cannot amend its req by identity.`); return; }

    if (await isClaudeInstalled()) {
        await launchClaude(root, 'Loom: Amend Requirements',
            `Loom amend requirements task. weave_slug="${weaveSlug}", thread_ulid="${threadUlid}". First READ the current req (its existing handles are AUTHORITATIVE — keep them verbatim). Re-read the thread's chat context (loom://context/thread/${weaveSlug}/${threadSlug}?mode=chat), then call MCP tool loom_amend_req ONCE with weave_slug="${weaveSlug}" thread_ulid="${threadUlid}" and content — ${REQ_BODY_SHAPE} APPEND-ONLY RULES (the tool refuses any other change): preserve every existing handle id and its number EXACTLY; add new scope only as fresh handles continuing the numbering (e.g. if IN1–IN6 exist, new items are IN7, IN8, …); NEVER renumber, reuse, or delete a handle; to retire an obsolete requirement, keep its line and mark it by inserting \`~dropped\` immediately after the handle (e.g. \`- \\\`IN3\\\` ~dropped superseded by IN7\`). This re-opens the req to draft and bumps its version.`,
        );
    } else {
        try {
            const result: any = await getMCP(root).callTool('loom_amend_req', { weave_slug: weaveSlug, thread_ulid: threadUlid });
            treeProvider.refresh();
            if (result?.filePath) {

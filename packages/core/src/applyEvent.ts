...
export function applyEvent(weave: Weave, event: WorkflowEvent): ApplyResult {
    ...
    for (let i = 0; i < updatedDocs.length; i++) {
        ...
        if (doc.type === 'plan') {
            ...
            if (
                ['ACTIVATE_PLAN', 'START_IMPLEMENTING_PLAN', 'COMPLETE_STEP', 'UPDATE_STEP', 'ADD_STEP', 'REMOVE_STEP', 'REORDER_STEPS', 'RECORD_RELEASE', 'FINISH_PLAN', 'BLOCK_PLAN', 'UNBLOCK_PLAN', 'CANCEL_PLAN'].includes(event.type)
            ) {
